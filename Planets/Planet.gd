...
@export var PlanetID = 0

enum PLANET_TYPES {TEMPERATE, FROZEN, MOLTEN, GAS, STAR, BLACK_HOLE}

@export var Connections : Array[Planet] = [] # All connected planets

@export var planet_type = PLANET_TYPES.TEMPERATE
...
func _ready():
	...
	# This is replicated in everything except Star.gd, which overrides _ready()
	get_node("Area2D").input_event.connect(_was_clicked)

func _was_clicked(viewport, event: InputEvent, shapeidx):
	if event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT:
			print("clicked on " + str(name))
			print("Planet Type: " + str(planet_type))
